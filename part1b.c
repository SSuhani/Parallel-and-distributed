...
void Compute_force(double loc_masses[],vect_t loc_forces[],vect_t loc_pos[], int loc_n);

void Update_part(int loc_part, double loc_masses[], vect_t loc_forces[],
      vect_t loc_pos[], vect_t loc_vel[], int loc_n, double delta_t) ;
...
// void ringUpdate (int loc_n, vect_t* pos);
     ...
     /*
      idea for drafting
      // the set up
      current = my_block // this is the processor
      current_rank = my_rank
      next= (my_rank + 1) % comm_sz
      previous= (my_rank -1 + comm_sz )% comm_sz

      // loop idea
      repeat (comm_sz - 1) times:
      send( current) to next;
      recieve (into : incoming) from previous
      current = incoming
      //owner tracking
...
int main(int argc, char* argv[]) {
   ...
   // not needed anymore since its all and we need local vect_t* pos;                /* Positions of all particles */
...
#  endif
   for (step = 1; step <= n_steps; step++) {
      t = step*delta_t;
      Compute_force(loc_masses, loc_forces, loc_pos, loc_n);
         for ( loc_part =0; loc_part<loc_n;loc_part++){
            Update_part(loc_part, loc_masses, loc_forces, loc_pos, loc_vel, loc_n, delta_t);
...
void Get_init_cond( double loc_masses[], vect_t loc_pos[], vect_t loc_vel[], int n, int loc_n){
 ...
 MPI_Scatter(all_masses, loc_n, MPI_DOUBLE, loc_masses, loc_n, MPI_DOUBLE, 0, comm);
   MPI_Scatter(all_pos, loc_n, vect_mpi_t, loc_pos, loc_n, vect_mpi_t, 0, comm);
...
double* incoming_masses= malloc(loc_n*sizeof(double));
vect_t* incoming_pos =malloc(loc_n*sizeof(vect_t));
double* current_masses= loc_masses;
vect_t* current_pos = loc_pos;

next = (my_rank+1)%comm_sz;
previous=  (my_rank-1 +comm_sz) %comm_sz;

for ( int i=0; i<loc_n;i++){
   loc_forces[i][X] = loc_forces[i][Y] = 0.0;
...
for ( int i =0; i<loc_n; i++){
   for (int j=0;j<loc_n;j++){
      if (i!=j){
         f_part_k[X] =loc_pos[i][X]-loc_pos[j][X];
         f_part_k[Y] =loc_pos[i][Y]-loc_pos[j][Y];
         len = sqrt(f_part_k[X]*f_part_k[X]+f_part_k[Y]*f_part_k[j] );
         len_3= len*len*len;
         mg= -G*loc_masses[i]*loc_masses[j];
         fact= mg/len_3;

         loc_forces[i][Y] = loc_forces[i][Y]+f_part_k[Y]*fact;
         loc_forces[i][X] = loc_forces[i][X]+f_part_k[X]*fact;
      }
   }
}

for (stage =0; stage<comm_sz-1; stage++){
   MPI_Sendrecv(current_masses, loc_n, MPI_DOUBLE, next, 0, incoming_masses, loc_n, MPI_DOUBLE, previous, 0, comm, MPI_STATUS_IGNORE);
   MPI_Sendrecv(current_pos, loc_n, vect_mpi_t, next, 1, incoming_pos, loc_n, vect_mpi_t, previous, 1,comm, MPI_STATUS_IGNORE);


   for (int i=0; i<loc_n;i++) {
      for (int j =0; j<loc_n; j++){
         f_part_k[X] = loc_pos[i][X] - incoming_pos[j][X];
         f_part_k[Y] = loc_pos[i][Y] - incoming_pos[j][Y];
         len = sqrt(f_part_k[X]*f_part_k[X] + f_part_k[Y]*f_part_k[Y] );
         len_3= len*len*len;
         mg= -G*loc_masses[i]*incoming_masses[j];
         fact = mg/len_3;
         loc_forces[i][X] = loc_forces[i][X] + f_part_k[X]*fact;
         loc_forces[i][Y] = loc_forces[i][Y]+ f_part_k[Y]*fact;
   ...
   current_masses =incoming_masses;
 ...
 *    and the position.
 */
void Update_part(int loc_part, double loc_masses[], vect_t loc_forces[], vect_t loc_pos[], vect_t loc_vel[], int loc_n, double delta_t) {



   double fact= delta_t/loc_masses[loc_part];
#  ifdef DEBUG
